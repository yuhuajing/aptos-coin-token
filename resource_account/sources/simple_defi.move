...
//921e42fd917f49e40f9b6299d9bcbb36f683c344c818e128b38b49cfb8c6ad95
...
module resource_account::simple_defi {
    ...
    fun exchange_to(a_coin: Coin<AptosCoin>): Coin<ChloesCoin> acquires ModuleData {
    ...
    fun exchange_from(c_coin: Coin<ChloesCoin>): Coin<AptosCoin> acquires ModuleData {
    ...
    public entry fun test_exchange_to_and_exchange_from(origin_account: signer, resource_account: signer, framework: signer) acquires ModuleData {
        ...
        let five_a_coins = coin::mint(500, &aptos_coin_mint_cap);
        coin::register<AptosCoin>(&origin_account);
        coin::register<ChloesCoin>(&origin_account);
        coin::deposit(signer::address_of(&origin_account), five_a_coins);
        assert!(coin::balance<AptosCoin>(signer::address_of(&origin_account)) == 500, 3);
        exchange_to_entry(&origin_account,20);
        assert!(coin::balance<AptosCoin>(signer::address_of(&origin_account)) == 480, 3);
        assert!(coin::balance<ChloesCoin>(signer::address_of(&origin_account)) == 20, 3);
