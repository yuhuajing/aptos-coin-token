...
module aptos_framework::multisig_account {
    ...
    // multisig_account is calculated based on the provided sig_addr and its nonce
   //    let owner_nonce = account::get_sequence_number(address_of(owner));
  //      let (multisig_signer, multisig_signer_cap) = account::create_resource_account(owner, create_multisig_account_seed(to_bytes(&owner_nonce))); 
    ...
    public entry fun create(  // with no additional owner
        owner: &signer,
        num_signatures_required: u64,
        metadata_keys: vector<String>,
        metadata_values: vector<vector<u8>>,
    ...
    public entry fun test_end_to_end(
        ...
        let owner = owners(multisig_account);
        debug::print(&owner);
        ...
        execute_rejected_transaction(owner_2, multisig_account);
        assert!(get_pending_transactions(multisig_account) == vector[
            get_transaction(multisig_account, 3),
        ], 0);
