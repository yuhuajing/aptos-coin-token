...
module YHJCoin::yhj_coin {
   ...
   // use YHJCoin::acl::{Self, ACL};
    ...
    fun team_emission(admin: &signer) acquires Caps, LockedYHJ {
        let count = 1;
        while (count <= 16) {
    ...
    public fun wl_mint_YHJ(
        ...
        assert!(is_allowed(&caps.acl,&wl_addr),E_NOT_WHIIELIST);
        register(wl);
    ...
    public fun is_allowed_Addr(addr: &signer):bool acquires Caps {
       ...
       is_allowed(&config.acl,&account_addr)
    }
